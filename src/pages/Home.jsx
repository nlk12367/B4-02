    ...
    <main className="absolute inset-0 w-full h-full flex flex-col dreamy-gradient overflow-hidden">
      ...
      <header className="w-full top-0 sticky z-50 bg-slate-50/50 dark:bg-slate-900/50 backdrop-blur-xl shadow-[0_40px_40px_rgba(160,45,112,0.06)]">
        <div className="flex items-center justify-between px-6 py-4 w-full">
          ...
          <button className="w-10 h-10 flex items-center justify-center text-on-surface-variant/60 hover:opacity-80 transition-opacity active:scale-95 transition-calm hover:scale-105">
            <span className="material-symbols-outlined" data-icon="settings">settings</span>
          </button>
        </div>
      </header>

      {/* Ambient Orbs (Design System Signature) */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-tertiary-container/20 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-secondary-container/20 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center -mt-12">
        <div className="relative flex flex-col items-center">
          <div className="iridescent-sphere w-64 h-64 md:w-80 md:h-80 rounded-full flex items-center justify-center animate-breathing transition-transform duration-300 active:scale-95">
            {/* Engraved Name */}
      ...
      {/* Floating Start Chat Bubble (Preserved from Screen 8) */}
      <div className="absolute bottom-32 left-0 right-0 flex justify-center z-40">
        <Link to="/chat" className="group relative flex items-center justify-center p-5 rounded-full bg-surface-container-lowest/40 backdrop-blur-2xl border border-white/20 shadow-[0_0_30px_rgba(160,45,112,0.1)] active:scale-95 transition-all duration-500 transition-calm hover:scale-105">
